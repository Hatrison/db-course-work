...
exports.add = (req, res) => {
  ...
  if (!(username && mail && id))
    return res.status(400).json("Username, mail and id are required");
...
exports.update = (req, res) => {
  ...
  if (!(username || mail))
    return res.status(400).json("Required username or mail");
  db.query(`SELECT * FROM user WHERE id=${id}`, (err, result) => {
    if (err) return res.status(500).json(err);
    if (result.length === 0)
    ...
    if (mail) {
      let query = `UPDATE user SET mail='${mail}' WHERE id=${id}`;
      db.query(query, (err) => {
        if (err) return res.status(500).json(err);
      });
    }
    res.status(200).json(`User with id ${id} was updated`);
  });
};
